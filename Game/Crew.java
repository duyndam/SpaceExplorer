...
public class Crew
{
  ...
  public String get_SerializedItemList()
  {
	  String strSerializedItems = "";
	  for(int indexInventory = 0; indexInventory < inventory.size(); indexInventory++)
	  {
		  
		  if(indexInventory == inventory.size()-1)
		  {
			  strSerializedItems += inventory.get(indexInventory).toString();
		  }
		  else
		  {
			  strSerializedItems += inventory.get(indexInventory).toString() + ",";
